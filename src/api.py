...
def load_model_for_current_alias(redownload_artifacts: bool = True) -> str:
    ...
    client = (
        MLFLOW_CLIENT
        if "MLFLOW_CLIENT" in globals() and MLFLOW_CLIENT
        else MlflowClient()
    )
    ...
    if redownload_artifacts:
        ...
        try:
            ...
            client.download_artifacts(
                run_id, "artifacts/preprocessing", dst_path=download_root
            )
    ...
    if os.path.isdir(nested):
        try:
            ...
            if not any(os.scandir(local_artifact_dir)):
                ...
                logger.warning(
                    "Nested preprocessing directory present but parent not empty; using parent."
                )
        except Exception as e:
            logger.warning(
                f"Could not flatten nested preprocessing dir: {e}. Using nested path."
            )
    ...
    if not isinstance(names, (list, tuple)):
        raise ValueError(
            f"Feature names file must contain list/tuple; got {type(names)}"
        )
    ...
    target_transformer = (
        joblib.load(tt_path) if tt_path and os.path.exists(tt_path) else None
    )
